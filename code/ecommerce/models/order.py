...
class OrderRegistration(object):
    ...
    def generate_delivered_orders(self, num_orders, execution_date_str=None):
        """
        Hàm sinh đơn hàng với trạng thái mặc định là DELIVERED (id=4).
        Dùng để test ETL flow mà không cần chạy qua Transaction update.
        """
        print(f"Bắt đầu tạo {num_orders} đơn hàng ĐÃ GIAO (Delivered) cho ngày {execution_date_str}...")

        if execution_date_str:
            base_date = datetime.strptime(execution_date_str, '%Y-%m-%d')
        else:
            # Nếu không truyền ngày, lấy giờ hiện tại
            base_date = datetime.now()

        # 1. Lấy dữ liệu nền (Users, Staff, Products...)
        self.cur.execute("""
            SELECT u.id, a.id 
            FROM users u
            JOIN addresses a ON u.id = a.user_id
        ...
        # 2. Lấy các ID trạng thái ĐÍCH DANH (Delivered / Completed)
        # ---------------------------------------------------------
        # Lấy ID trạng thái đơn hàng là 'Delivered' (Thay vì Pending)
        self.cur.execute("SELECT id FROM orderstatus WHERE order_status_name='Delivered'")
        delivered_order_status_id = self.cur.fetchone()[0]

        # Lấy ID trạng thái thanh toán là 'Completed' (Cho logic hợp lý)
        self.cur.execute("SELECT id FROM paymentstatus WHERE payment_status_name='Completed'")
        completed_payment_status_id = self.cur.fetchone()[0]
        
        # Lấy ID trạng thái vận chuyển là 'Delivered'
        self.cur.execute("SELECT id FROM shippingstatus WHERE shipping_status_name='Delivered'")
        delivered_shipping_status_id = self.cur.fetchone()[0]
        # ---------------------------------------------------------
        ...
        for _ in range(num_orders):
            ...
            # Xử lý thời gian
            if execution_date_str:
                # Nếu chạy backfill, rải đều trong ngày
                created_at = base_date + timedelta(seconds=random.randint(0, 86399))
            else:
                # Nếu chạy realtime test, lấy giờ hiện tại (trừ đi vài giây để an toàn)
                created_at = pendulum.now("Asia/Ho_Chi_Minh").naive() - timedelta(seconds=random.randint(10, 60))
            
            # Gom dữ liệu bảng Orders (Sử dụng các ID Delivered/Completed đã lấy ở trên)
            order_tuple = (
                customer_id, staff_id, address_id, order_amount, discount_amount, tax_amount,
                total_amount, discount_id, payment_id, 
                completed_payment_status_id,  # Payment -> Completed
                delivered_order_status_id,    # Order Status -> Delivered
                shipping_id, 
                delivered_shipping_status_id, # Shipping Status -> Delivered
                created_at
            )
            orders_data_list.append(order_tuple)
            order_details_temp_storage.append((len(orders_data_list) - 1, line_items, created_at))
        ...
        try:
            ...
            self.conn.commit()
            print(f"✅ THÀNH CÔNG! Đã tạo {len(inserted_order_ids)} đơn hàng trạng thái DELIVERED.")
            return [row[0] for row in inserted_order_ids]
